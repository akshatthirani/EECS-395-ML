...
if strcmp(kernel, 'rbf')
    H = generate_kernel(D(2:end,:), 'rbf', varargin{6:end});     % RBF Kernel
elseif strcmp(kernel, 'poly')
    H = generate_kernel(D(2:end,:), 'poly', varargin{6:end});    % Polynomial Kernel
else
...
%% Configure gradient
soft_svm_gradient = @(y)(-2.*D*diag(b)*max((ones(N,1)-diag(b)*D'*y),zeros(N,1)) + 2.*lambda.*U*y);
